...
class ViewController: UIViewController {
    ...
    func animateTransitionIfNeeded(state: CardState, duration:TimeInterval) {
        if runningAnimations.isEmpty {
            // For View Itself
            let frmameAnimator = UIViewPropertyAnimator(duration: duration, dampingRatio: 1) {
                switch state {
                case .expanded:
            ...
            frmameAnimator.addCompletion { _ in
                self.cardVisible = !self.cardVisible
                self.runningAnimations.removeAll()
            }
            frmameAnimator.startAnimation()
            runningAnimations.append(frmameAnimator)
